...
propongono un'architettura innovativa che integra le due architetture
...
Uno dei primi lavori ad affrontare il problema per quanto riguarda i
\textit{Vision Transformer} è stato quello di Chen et al.
...
\subsubsection{ViTAS}
Un approccio radicalmente diverso è quello proposto da Su et al. con ViTAS, un framework nato dalla constatazione che l'applicazione diretta del NAS classico ai Vision Transformer produce spesso risultati instabili o fallimenti catastrofici durante l'addestramento del Superformer (la versione Transformer della SuperNet). Secondo gli autori, la causa risiede nella mancanza di pregiudizi induttivi (inductive bias) tipica dei ViT: l'impiego di canali sbilanciati per architetture diverse compromette l'efficacia del weight sharing.Per stabilizzare questo processo, il lavoro introduce tre pilastri tecnologici:Il Meccanismo di Cyclic Weight-SharingIl problema principale riscontrato nel NAS tradizionale è il cosiddetto bias di valutazione dei canali. Nelle tecniche comuni (come AutoSlim), si utilizza un meccanismo ordinale: se una sottorete necessita di $i$ canali, vengono presi i primi $i$ da sinistra. Ciò significa che i canali posizionati all'inizio della matrice vengono addestrati molto più spesso di quelli finali, creando una disparità enorme nella loro ottimizzazione.
ViTAS risolve questo problema con il Cyclic Weight-Sharing:Fairness dell'addestramento: ogni canale deve ricevere lo stesso numero di aggiornamenti indipendentemente dalla dimensione della sottorete campionata.Uniformità dell'influenza: ogni canale deve avere un impatto equivalente sul gradiente totale.
